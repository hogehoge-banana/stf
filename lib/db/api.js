...
dbapi.insertSchedule = (serial, email, start, end) => {
...
dbapi.updateSchedule = (id, start, end) => {
  return db.run(
    r.table('schedules').get(id).update({
      start: start
    , end: end
    })
  )
...
dbapi.removeSchedule = (id) => {
  return db.run(
    r.table('schedules').get(id).delete()
  )
}
