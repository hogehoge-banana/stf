...
module.exports = function(options) {
  ...
  return syrup.serial()
    ...
    .dependency(require('./support/app'))
    .dependency(require('./support/dev'))
    .define(function(options, sub, app, dev) {
      ...
      function onScheduleChange(changeFeed) {
        ...
        .spread(function(device, message) {
          var enveloped = wireutil.envelope(message)
          app.send([
            device.channel, enveloped])
          dev.send([
            wireutil.global, enveloped])
        })
      }
      ...
      lifecycle.observe(function() {
        [app, dev, sub].forEach(function(sock) {
          try {
            sock.close()
          }
          catch (err) {
